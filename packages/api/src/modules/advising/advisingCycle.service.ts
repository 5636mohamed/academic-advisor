...
export async function runAdvisingCycle(student: StudentWithCgpa, ports: AdvisingCyclePorts): Promise<AdvisingActionResult> {
  ...
  // they were predicted to outright fail): never filtered here directly —
  // this app has TWO independent planners (this one, and server.ts's
  // buildScoredPlan for the Fastest-Graduation/Target-CGPA tabs) that both
  // funnel into packPlan, and the F-exclusion filter lives there once, so
  // neither this nor any future third planner can reintroduce the gap by
  // forgetting to filter its own pool first. See planPacker.ts's packPlan
  // for the actual filter and the full explanation.
